...
## Verify a canvas move from the data delta, and know your snapshot's refresh cadence
- **What went wrong (nearly):** after a move, a territory's hover tooltip still read "no units" — looked like the move failed. It hadn't: the `StateSnapshot` (the browser's `units` map) only refreshes at **step** boundaries, while the engine's `UnitCollection` updates on every accepted move. The authoritative live signal is the *next move request's* `movableUnits`, not the snapshot.
- **Rule:** Confirm a mid-phase canvas action from the freshest authoritative data (here: re-select the source and read the unit count delta — destroyer 2→1, infantry 6→4), not from a projection that updates on a coarser cadence. Know which feed updates when before trusting it as a verification oracle.

...
- [x] **Undo:** `MoveRequest.undoableMoves` lists this phase's moves (`UndoableMoveInfo{index,label,canUndo}` from `IMoveDelegate.getMovesMade()`); reply `{undo:index}` → `delegate.undoMove(index)` → re-broadcast (units snap back). `MovePanel` shows an Undo button per move (disabled when a later move depends on it). Zero engine change.

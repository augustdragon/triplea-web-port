 ...
 * back a delegate rejection of a prior submission. The browser replies either {@code {done:true}}
 * (end the phase) or {@code {route:[territoryNames], units:{type:count}}} (one move to perform;
 * units are drawn from the first territory in the route; a land→sea route auto-loads the chosen
 * land units onto transports in the destination sea zone).
 */
